...
function assembleExcitationVector!(
    ...
    fill!(V, zero(Complex{FT}))
    return excitationVectorMFIE(ports, trianglesInfo, nbf; strategy = mfie_strategy)
end
...
function assembleExcitationVector!(
    ...
    fill!(V, zero(Complex{FT}))
    return excitationVectorCFIE(ports, trianglesInfo, nbf;
                                alpha = formulation.alpha,
                                mfie_strategy = mfie_strategy)
end
...
function computeInputImpedance(
    ...
    if abs(I_port) < eps(FT)
        error("Zero current at port $(port.id)")
    end

    # 对于电流探针，我们使用探针电流 I 作为端口电流
    # 端口电压通过 V = Z * I 计算
    # 然后 Z_in = V_port / I_port = (Z * I)_port / I_port
    # 这里使用激励向量中的电流值作为端口电流
    Z_in = port.I / I_port
...
function computeSParameters(
    ...
    # 获取所有端口的编号
    port_ids = [port.rwgID for port in ports.ports if isa(port, DeltaGapPort) || isa(port, CurrentProbe)]

    # 验证端口有效性
    if any(pid <= 0 || pid > nbf for pid in port_ids)
        error("Invalid port rwgID in port array")
    end

    # 构建端口阻抗矩阵 Z_port
    # Z_port[i,j] = V_i / I_j，当端口 j 激励，端口 i 测量电压
    ...
    for j in 1:num_ports
        ...
        if isa(port_j, DeltaGapPort) || isa(port_j, CurrentProbe)
            excitationVectorEFIE!(V_j, port_j, TriangleInfo{IT, FT}[])
        end
        ...
        try
            ...
            for i in 1:num_ports
                ...
                if isa(port_i, DeltaGapPort) || isa(port_i, CurrentProbe)
                    pid_i = port_i.rwgID
                    # Z_ij = V_i / I_j
                    if abs(I_j[pid_i]) > eps(FT)
                        Z_port[i, j] = port_i.V / I_j[pid_i]
                    else
    ...
    # 计算 S 参数矩阵: S = (Z - Z0*I) * (Z + Z0*I)^(-1)
    Z0_matrix = Z0 * Matrix{FT}(I, num_ports, num_ports)
    ...
    try
        ...
        # 如果矩阵求逆失败，返回 NaN 矩阵
        S_matrix = Matrix{Complex{FT}}(NaN*ones(num_ports, num_ports))
    end
